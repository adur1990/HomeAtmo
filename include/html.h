...
// BODY
   "<body>\n"
    "<h2>HomeAtmo</h2>\n"
        ...
        "<br />"
        "<br />"
        "<div><h3>Luftqualität</h3><canvas id=\"chart-iaq_avg\"></canvas><p id=\"iaq_avg\">0</p></div>\n"
        "<br />"
        "<br />"
...
// SCRIPT
    "<script>\n"
        ...
        "press_request.send();"
        // IAQ AVG
        "var iaq_avg_target = document.getElementById('chart-iaq_avg');"
        "var iaq_avg_val = document.getElementById('iaq_avg');"
        "var iaq_avg_gauge = new Gauge(iaq_avg_target).setOptions(opts);"
        "iaq_avg_gauge.maxValue = 300;"
        "iaq_avg_gauge.minValue = 0;"

        "var iaq_avg_request = new XMLHttpRequest();"
        "iaq_avg_request.onreadystatechange = function() {"
            "if (this.readyState == 4 && this.status == 200) {"
                "var y = parseFloat(this.responseText);"
                "iaq_avg_gauge.set(y);"
                "iaq_avg_val.innerHTML = y;"
            "}"
        "};"
        "iaq_avg_request.open(\"GET\", \"/iaq_avg\", true);"
        "iaq_avg_request.send();"
        // IAQ AVG
        ...
        "iaq_request.onreadystatechange = function() {"
            "if (this.readyState == 4 && this.status == 200) {"
                "var y = parseFloat(this.responseText);"
                "var iaq_avg_val = document.getElementById('iaq_avg');"
                "document.getElementById('iaq_avg').innerHTML = iaq_avg_val.innerHTML + \", \" + y;"
            "}"
        "};"
